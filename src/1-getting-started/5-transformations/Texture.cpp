...
{
  stbi_set_flip_vertically_on_load(1); // opengl expects the texture to start at the bottom left, not the top left, with this we invert this
                                       // 4 because we want RGBA
...
void Texture::Bind(unsigned int slot) const
{
  ...
  // but usually there is ways to reuse texture, so you wont use all slo ts
