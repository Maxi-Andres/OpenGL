...
int main()
{
  ...
  //! Scope ensures proper destruction order of OpenGL objects before context is destroyed
  {
    ...
    glm::vec3 lightTranslation = glm::vec3(200.0f, 0.0f, 0.0f);
    ...
    Shader shader("../../src/2-lighting/1-colors-and-2-basic-lighting/1.2-orbiting-cubes/shaders/Shaders.shaders");
    shader.Bind();
    ...
    shader.SetUniform1f("u_Scale", 1.2f); //! Scale
    ...
    // ======================= LIGHT =======================
    Shader lightShader("../../src/2-lighting/1-colors-and-2-basic-lighting/1.2-orbiting-cubes/shaders/LightShaders.shaders");
    lightShader.Bind();
    ...
    // rotation speeds
    float cubeRotationSpeed = 30.0f; // Cube rotation speed (degrees/second)
    float lightOrbitSpeed = 45.0f;   // Speed of light in orbit (degrees/second)

    float orbitRadius = 200.0f;
    bool lightOrbitingBool = true;
    glm::vec3 lightOrbiting = glm::vec3(0.0f, 0.0f, 0.0f);
    ...
    while (!glfwWindowShouldClose(window))
    {
      ...
      updateTitle(window, "Orbiting Cubes"); //* This is only for the fps
      ...
      deltaTime = static_cast<float>(crntTime - prevTime); // Time since last frame
      prevTime = crntTime;

      // ===== CUBE ROTATION (frame-independent) =====
      if (modelRotationBool)
      {
        if (rotationX)
          modelRotation.x += cubeRotationSpeed * deltaTime; // Multiply by deltaTime so that no matter how many frames there are, it will always rotate by the same number of degrees.
        if (rotationY)
          modelRotation.y += cubeRotationSpeed * deltaTime;
        if (rotationZ)
          modelRotation.z += cubeRotationSpeed * deltaTime;
      }

      // ===== LIGHT ORBITING (frame-independent) =====
      if (lightOrbitingBool)
      {
        lightOrbiting.x += lightOrbitSpeed * deltaTime;
        lightOrbiting.y += lightOrbitSpeed * deltaTime;
        lightOrbiting.z += lightOrbitSpeed * deltaTime;
      }
      ...
      //! this is not a good way of rendering a lot of things, what you want to do is batch rendering in a single draw call, calling multiple times drawing is bad!
      {
        ...
        // lightModel = glm::translate(lightModel, lightOrbiting);

        // Calcula posición orbital usando trigonometría
        glm::vec3 orbitCenter = glm::vec3(0.0f, 0.0f, 0.0f);

        glm::vec3 finalLightPos = orbitCenter + glm::vec3(
                                                    orbitRadius * cos(glm::radians(lightOrbiting.x)),
                                                    orbitRadius * sin(glm::radians(lightOrbiting.y)),
                                                    orbitRadius * sin(glm::radians(lightOrbiting.z)));

        lightModel = glm::translate(glm::mat4(1.0f), finalLightPos);
        lightModel = glm::rotate(lightModel, radY, glm::vec3(0.0f, -1.2f, 0.0f));
        lightModel = glm::rotate(lightModel, radX, glm::vec3(-1.2f, 0.0f, 0.0f));
        lightModel = glm::rotate(lightModel, radZ, glm::vec3(0.0f, 0.0f, -1.2f));

        mvp = proj_view * lightModel;
        ...
        // IMPORTANTE: Envía la posición final al shader
        shader.Bind();
        shader.SetUniform3f("u_LightTranslation", finalLightPos.x, finalLightPos.y, finalLightPos.z);
      ...
      // ========================= ImGui =========================
      {
        ...
        if (ImGui::CollapsingHeader("Auto Rotation"))
        {
          ...
          ImGui::DragFloat("Rotation Speed (°/s)", &cubeRotationSpeed, 1.0f, 0.0f, 720.0f);
        ...
        {
          float buttonWidth = ImGui::GetContentRegionAvail().x * 0.5f - 5.0f;
          if (ImGui::Button("Enable Orbiting", ImVec2(buttonWidth, 0)))
            lightOrbitingBool = true;
          ImGui::SameLine();
          if (ImGui::Button("Disable Orbiting", ImVec2(buttonWidth, 0)))
            lightOrbitingBool = false;

          ImGui::DragFloat("Orbit Radius", &orbitRadius, 1.0f, 50.0f, 1000.0f);
          ImGui::DragFloat("Orbit Speed (°/s)", &lightOrbitSpeed, 1.0f, 0.0f, 720.0f);
